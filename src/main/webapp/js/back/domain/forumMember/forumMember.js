	...
	$('#datagrid').datagrid({
		...
		[
	        ...
	        {field:'operate',title:'操作',width:160,align:'center',formatter: function(value,row,index){
	        	return  '<span class="g_alive"><span  style="cursor: pointer;color: #EC4949" onclick="openForumMemberDetailWin('+index+',\'update\')">修改</span>'
	        	+'&nbsp;|&nbsp;<span class="update" style="cursor: pointer;color: green" onclick="openForumMemberDetailWin('+index+',\'read\')">查看</span></span>'
	    ...
	    onDblClickRow: function(index, row){
	    	openForumMemberDetailWin(index, 'read');
        },
...
function batchDelete(){
	commonBatchDelete('/back/updateForumMemberDelete');//common_back:通用批量恢复
}

/**批量彻底删除**/
function batchThoroughDelete(id){
	commonBatchThoroughDelete('/back/deleteForumMemberThorough',null,id);//common_back:通用批量强制删除
}

/**批量恢复数据**/
function batchRecover(id){
	commonBatchRecover('/back/updateForumMemberRecover',null,id);//common_back:通用批量恢复
}

/**添加数据*/
function addForumMember(){
	openForumMemberDetailWin(-1,'add');
}

function openForumMemberDetailWin(index,operation){
	var rows,data;
	if(index >= 0){
		rows = $('#datagrid').datagrid('getRows');
		data = rows[index];//获取该行的数据
	}
	var $form = $('#forumMemberDetailForm');
	
	$form.form({
	    url:'',
	    onSubmit: function(){
	       $('#forumMemberDetailForm').form('enableValidation');
	       var flag = $('#forumMemberDetailForm').form('validate');
	       if(flag){
	       		var id;
	       		if(operation != 'add')id = data.id;
	       		var forumMember = getForumMemberDetail(id);//得到需要的字段信息
	       	    $.post('/back/addOrUpdateForumMember',forumMember,function(rsp){
					 if(rsp.success){
						 $('#datagrid').datagrid('reload');
						 //$('#datagrid').datagrid('clearSelections');
						 //$('#datagrid').datagrid('clearChecked');
				       	 setTimeout(function(){
				       		closeForumMemberDetailWin();
				       	 },500);
					 }else{
	...
	loadCombo();
	$('#forumMemberDetailWin').window('open');
	$form.form('clear');
	$form.form('disableValidation');
	$('#forumMemberDetailTip').html('');
	
	var domIds = "#f_user,#f_forum,#f_mainStatus";
	
	if(operation == 'add'){//添加
		$('#winSearchTr,#winSearchDivisionTr').show();//显示查询按钮
		$('#forumMemberDetailTable .readOnlyTr').hide();//隐藏只读字段
		$('#forumMemberDetailSave').show();
		$(domIds).textbox('readonly',false);//新增时设置为可编辑
	}else{//查看或修改
		$('#forumMemberDetailTable .readOnlyTr').show();//显示只读字段
		$('#winSearchTr,#winSearchDivisionTr').hide();//隐藏查询按钮
		var isReadOnly = false;
		if(operation == 'read'){//查看
			isReadOnly = true;
			$('#forumMemberDetailSave').hide();
		}else{//修改
			$('#forumMemberDetailSave').show();
		}
		$(domIds).textbox('readonly',isReadOnly);//设置只读还是可编辑
		setForumMemberDetail(data);//设置用户详细信息字段值
	}
}

/** 得到相关信息的字段值 */
function getForumMemberDetail(id){
	//var mainStatus = $('#f_mainStatus').combobox('getValue');
	var userId = $('#f_userId').val();
	var forumId = $('#f_forum').combobox('getValue');
...
//提交数据
function submitForumMemberDetail(){
	//$('#forumMemberDetailForm').submit();
	var timeRec = preventRepeat(10, SAVING_FLAG);
	if(timeRec){
		SAVING_FLAG = timeRec;
		$('#forumMemberDetailForm').submit();//执行操作
	}else{//重复提交
		return;//可进行提示或其他操作，这里直接返回，即重复提交时没有反应
	}
}
//关闭窗口
function closeForumMemberDetailWin(){
	$('#forumMemberDetailWin').window('close');
}

/**读取删除数据*/
function showTrash(){
	IS_SHOW_TRASH = true;
	commonShowTrash('#batchRecover,#batchThoroughDelete,#returnBack','#batchDelete,#addForumMember,#showTrash');
}
/**读取未删除数据**/
function returnBack(){
	IS_SHOW_TRASH = false;
	commonReturnBack('#batchDelete,#addForumMember,#showTrash','#batchRecover,#batchThoroughDelete,#returnBack');
}
