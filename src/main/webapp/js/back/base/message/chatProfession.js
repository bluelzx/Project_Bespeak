/** 基础数据和基础设置 */
lh.config = {
	mainObjLowerName : 'chatProfession',
	mainObjUpperName : 'ChatProfession',
}
...
function loadGrid(){
	lh.$grid.datagrid({
		...
		[
	        ...
	        {field:'msgGroup',title:' 发送类型',width:120,align:'center',formatter:function(value,row,index){
	        	var type = '未定义';
	        	if(value == 1){ type = '(1)普通消息'; }
	        	else if(value == 2){ type = '(2)出价消息'; }
	        	else if(value == 3){ type = '(3)系统消息'; }
	        	else if(value == 4){ type = '(4)主持人消息'; }
	        	return type;
	        }},
	        ...
	        {field:'deletedAt',title:'删除时间',width:120,align:'center',formatter: function(value,row,index){
	        	return lh.formatGridDateTime({date:new Date(value)});
	        }},
