...
function openUserFundWin(index,operation){
	...
	var domIds = "#f_type,#f_money,#f_tradeType";
	...
	if(operation == 'read'){//查看
		isReadOnly = true;
		$('#userFundSave,#type,#newMoney,#tradeType').hide();
	}else{//修改
		$('#userFundSave').show();
	}
...
function getUserFund(id){
	...
	var type = $('#f_type').combobox('getValue');
	var money = $("#f_money").textbox('getValue');
	var tradeType = $("#f_tradeType").textbox('getValue');
	var userFund = {
