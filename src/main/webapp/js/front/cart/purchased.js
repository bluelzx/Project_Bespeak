...
function makeOrderGoodsListDom(orderGoodsList,isAppend){
	...
	for(var a in orderGoodsList){
		if(orderGoodsList[a].shippingStatus == 1){
			orderGoodsList[a].logisticsStatus = "<span>状态:<span style='color:red;'>未发货</span></span>";
		}else if(orderGoodsList[a].shippingStatus == 2){
			orderGoodsList[a].logisticsStatus = "<span>状态:<span style='color:red;'>已发货</span></span>";
		}else if(orderGoodsList[a].shippingStatus == 3){
			orderGoodsList[a].logisticsStatus = "<span>状态:<span>已收货</span></span>";
		}else if(orderGoodsList[a].shippingStatus == 4){
			orderGoodsList[a].logisticsStatus = "<span>状态:<span style='color:red;'>退货</span></span>";
		}
