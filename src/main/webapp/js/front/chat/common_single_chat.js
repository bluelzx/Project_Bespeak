...
function appendChatHistory(rows,userId,isShowHour){
	for(var i = 0;i<rows.length;i++){
		...
		if(c.senderId == userId){
			...
			c.receiverAvatar = c.senderAvatar;
			msgReceiveAppend(c);
		}
	}
}
...
function loadChatHistory(options){
	//return;//暂时不加载聊天记录
	var receiverId = options.receiverId;//发送人作为接收人：接收其他人发的消息
	var senderId = options.senderId;//,currentUserChat:"1"
	var param = {sc_order:'send_time___ASC',typeId:1,page:1,rows:10,twoPeopleChat:"twoPeopleChat"};
	if(receiverId)param.receiverId = receiverId;
	if(senderId)param.senderId = senderId;
