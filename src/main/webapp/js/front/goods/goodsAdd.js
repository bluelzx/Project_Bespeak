...
function setShowLength(obj, maxlength, id) { 
	...
	if(id='name_word'){
		document.getElementById(wid).innerHTML = "还可以输入" + rem + "字拍品名称"; 		
	}else{
		document.getElementById(wid).innerHTML = "还可以输入" + rem + "字拍品描述"; 
	}
}
...
function addGoods(paramObj){
	...
	lh.post('front', '/goods/addGoods', goods,function(rsp){
		if(rsp.success){
			lh.current.goodsId = rsp.goodsId;
			if(lh.param.pageFrom == 'ap' && lh.param.paramSerial){
				lh.confirm('您已成功上传拍品到专场，是否继续添加？', lh.reload, lh.back);
			}else if(lh.param.pageFrom == 'am'){
				//lh.confirm('您已成功上传藏品，是否将该藏品发布到微拍？', jumpToAmAddOrUpdate, lh.reload);
				jumpToAmAddOrUpdate();//微拍直接跳转不用提示
			}else{
...
function jumpToAmAddOrUpdate(){
	lh.jumpR('/am/page/addOrUpdate?goodsId='+lh.current.goodsId);
}
