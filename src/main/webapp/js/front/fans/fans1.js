...
$(function(){
	...
	lh.scrollBottom(scrollBottomTab);
});
...
function loadNoticeList(isShowHaveRead){
	var param = {rows:PAGE_COUNT,page:CURRENT_NOTICE_PAGE,orderBy:"read_status",ascOrdesc:"ASC"};//,notRead:1未读
...
function showNoticeAndRead(content,serial,linkUrl){
	if(linkUrl){
		frontToResource(content,"jumpToResource('"+linkUrl+"')","noticeReaded('"+serial+"')");
	}else{
		lh.alert(content,"noticeReaded('"+serial+"')");
	}
}
function showChatAndRead(content,serial){
	lh.alert(content,"readed('"+serial+"')");
}

function jumpToResource(url){
	window.location.href = url;
}

function frontToResource(content, yesFunName, cancelFunName){
	if(!content)return;
	if($confirmDialog.length<=0){
		var event = '$confirmDialog.hide();';
		if(yesFunName)event += yesFunName+';';
		var event2 = '$confirmDialog.hide();';
		if(cancelFunName)event2 += cancelFunName+';';
		var dom = 
			'<div class="weui_dialog_confirm" id="baseConfirmDialog" style="display: none;">'+
		        '<div class="weui_mask"></div>'+
		        '<div class="weui_dialog">'+
		            '<div id="baseDialog_title2" class="weui_dialog_hd"><strong class="weui_dialog_title">提示</strong></div>'+
		            '<div id="baseDialog_msg2" class="weui_dialog_bd"></div>'+
		            '<div class="weui_dialog_ft">'+
		                '<a href="javascript:;" onclick="'+event2+'" class="weui_btn_dialog default">取消</a>'+
		                '<a href="javascript:;" onclick="'+event+'" class="weui_btn_dialog primary">查看</a>'+
		            '</div>'+
		        '</div>'+
		    '</div>';
		$('body').append(dom);
