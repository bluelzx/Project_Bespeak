...
function initOption(option, random){
	...
	opt.chooseBtnDomId = option.chooseBtnDomId || null;
...
function initUploadSimple(option) {
	initUpload('/back/upload', option);//调用完整方法
}
...
function initUploadAllFile(option) {
	initUploadFile('/back/upload?folderType=2', option);//调用完整方法
}
