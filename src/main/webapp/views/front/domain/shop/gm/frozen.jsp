...
<body style="background-color: #f5f5f5;">
			...
			<div class="slide_shop" id="slide_shop">
				<div class="extension">
					<div style="text-align: center;padding-top: 5px;padding-left:0px;padding-right:0px;">
						...
						<a href="javascript:;" class="weui_btn weui_btn_primary"  onclick="$('.navcdbox').show();" >藏品分类：冻结∨</a><!-- width:48% -->
					</div>
				</div>
				<div class="pz_main">
	...
	<script type="text/javascript" src="/js/front/shop/gm/frozen.js" title="v"></script>
<%-- 	<c:if test="${moduleId != 5}">
		<button  type="button" class="btn btn-info" onclick="frozenGoods('{{id}}');return false;">解冻</button>(在买家3天内未付钱后,才可以解冻)
	</c:if>
	<c:if test="${moduleId == 5}">
		<button  type="button" class="btn btn-info" onclick="frozenGoods('{{wholesaleId}}');return false;">解冻</button>(在买家3天内未付钱后,才可以解冻)
	</c:if> --%>
